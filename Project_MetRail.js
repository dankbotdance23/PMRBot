...
Client.on("message",message=> {
    ...
    if(cmd === `${PREFIX}help`){
        ...
        .addField("Moderator Commands", "kick")
        .addField("Commands due next update", "ban, mute, softban, unban, unmute")
        .setTimestamp()
        .setColor("#f79f0b")
    ...
    if(cmd === `${PREFIX}credits`){
        ...
        .setTimestamp()
        .setColor("#f79f0b")
        .setFooter("y do u read dis xD")
        message.channel.send({embed: creditsembed})
    }
    
    if(cmd === `${PREFIX}kick`){
        if(!message.member.hasPermission(['VIEW_AUDIT_LOG'])) return message.reply("You do not have permissions to do this command 🤦")
